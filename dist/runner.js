...
export async function runValidation(options) {
    ...
    try {
        ...
        try {
            ...
            if (timeoutErr.message.includes('Timeout')) {
                if (verbose)
                    console.log(`  [warn] networkidle timed out — retrying with load+wait`);
    ...
    if (isBlocked) {
        await browser.close();
        throw new Error(`The page at ${url} returned an "Access Denied" response — the site's bot protection (e.g. Akamai, Cloudflare) is blocking the headless browser. Results would be unreliable; validation aborted.`);
    }
