 ...
 */
export async function runGtmChecks(page: Page, verbose: boolean): Promise<CheckResult[]> {
  const results: CheckResult[] = [];

  // ── Check 7: GTM fires after consent defaults ─────────────────────────────
  const gtmOrderResult = await page.evaluate((): { ok: boolean; detail: string } => {
    ...
    for (let i = 0; i < dl.length; i++) {
      ...
      if (entry[0] === 'consent' && entry[1] === 'default') {
        if (consentDefaultIndex === -1) consentDefaultIndex = i;
        continue;
      }
  ...
  const v2FieldsResult = await page.evaluate((): { ok: boolean; detail: string } => {
    const dl: unknown[] = (window as unknown as { dataLayer?: unknown[] }).dataLayer ?? [];
    ...
    for (const entry of dl) {
      ...
      if (e && e[0] === 'consent' && (e[1] === 'default' || e[1] === 'update')) {
        const params = e[2] as Record<string, unknown>;
