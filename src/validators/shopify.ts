 ...
 */
export async function runShopifyChecks(page: Page, verbose: boolean): Promise<CheckResult[]> {
  const results: CheckResult[] = [];

  // ── Check 5: Shopify.customerPrivacy loads before GTM ────────────────────
  const privacyApiResult = await page.evaluate((): { ok: boolean; detail: string } => {
    const shopify = (window as unknown as { Shopify?: Record<string, unknown> }).Shopify;
    ...
    const api = shopify.customerPrivacy as Record<string, unknown>;
    ...
    const gtmIndex = scripts.findIndex(s => /googletagmanager\.com\/gtm\.js/.test((s as HTMLScriptElement).src));
  ...
  const webPixelsResult = await page.evaluate((): { ok: boolean; detail: string } => {
    const shopify = (window as unknown as { Shopify?: Record<string, unknown> }).Shopify;
    if (!shopify?.customerPrivacy) {
    ...
    const api = shopify.customerPrivacy as Record<string, unknown>;
    let currentConsent: Record<string, unknown> = {};

    try {
      if (typeof api.currentVisitorConsent === 'function') {
        currentConsent = (api.currentVisitorConsent as () => Record<string, unknown>)();
      }
    } catch {
